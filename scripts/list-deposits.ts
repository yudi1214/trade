 ...
 * Útil para verificar quais depósitos estão disponíveis
 */
  ...
  .then((deposits) => {
    ...
    if (deposits) {
      ...
      if (deposits.length > 0) {
        ...
        console.log("2. Execute o script check-deposit-status-fixed.ts com o ID do depósito")
        console.log("   Exemplo: npx ts-node scripts/check-deposit-status-fixed.ts ID_DO_DEPOSITO")
      } else {
        console.log(
